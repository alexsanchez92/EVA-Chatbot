    ...
    $("#info-eva-close").click(function() {
        localStorage.setItem('infoEva', true);
        $( "#chat-bubble" ).fadeIn( "slow", function() {});
    });

    //Abrimos el chat desde la burbuja del modal info
    $("#bubble-info").click(function() {
        localStorage.setItem('infoEva', true);
        localStorage.setItem('chatEva', 1);
        $('#modalEvaInfo').modal('hide');
        $('#modalEva').modal({show: true, backdrop: false});
        callToEvent();
        first=true;
    });
...
$(document).ready(function() {
    ...
    if(!localStorage.getItem('infoEva')){
        $('#modalEvaInfo').modal({show: true, backdrop: false});
    }

    //Si está la variable "name" en la url. Si coincide mostramos el chat humano
    if(getUrlParam("chat")=="human"){
        ...
        c += "<div id=\"chatra-container\"></div><script>(function(d, w, c) {w.ChatraID = 'bnWr994iCcTWfyQi7';var s=d.createElement('script');w[c]=w[c] || function(){(w[c].q = w[c].q || []).push(arguments);};s.async=true;s.src='https://call.chatra.io/chatra.js';if (d.head) d.head.appendChild(s);})(document, window, 'Chatra');<\/script> <script>window.ChatraSetup = {colors: {buttonText: '#fff', buttonBg: '#006dff'}};Chatra('openChat')<\/script> <script>window.ChatraSetup = {chatWidth: 550,chatHeight: 650};<\/script><script>window.ChatraSetup = {    mode: 'frame',injectTo: 'chatra-container'};<\/script>";
        
        $("#chat-container").html(c);

        if(localStorage.getItem('chatEva')==1){
            $('#modalEva').modal({show: true, backdrop: false});
        }
        else{
            $( "#chat-bubble" ).fadeIn( "slow", function() {});
    ...
    else if(localStorage.getItem('chatEva')==1){
        $('#modalEva').modal({show: true, backdrop: false});
        callToEvent();
        first=true;
    }
    else if(localStorage.getItem('infoEva')){
        $( "#chat-bubble" ).fadeIn( "slow", function() {});        
    }
});

    //Cierra el chat del bot para mostrar la burbuja flotante
    $( "#eva-close" ).click(function() {   
        localStorage.setItem('chatEva', 0);        
        $( "#mytext" ).val('');
        $('#modalEva').modal({show: false});
        $( "#chat-bubble" ).fadeIn( "slow", function() {});
    });
