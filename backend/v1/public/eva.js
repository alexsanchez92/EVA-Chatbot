    ...
    var accessToken = "107098adbb7f4091b132167d9631079e";
    //var accessToken = "c88f17eeb9af4f1b9b556bb524498b6d"; //VERSION REAL
    var apiUrl = "https://chatbotsau-220511.appspot.com/v1/";
...
$(document).ready(function() {
    ...
    function callToEvent(rel){
        ...
        $.ajax({
            ...
            success: function(data) {
                console.log("RESP CODE: "+data.status.code)
                if(data.status.code==206 && reloadTry<3){
                    reloadTry++;
                    callToEvent(reloadTry);
                    return;
                }
        ...
        $.ajax({
            ...
            success: function(textResponse) {
                $.ajax({
                    ...
                    data: JSON.stringify({ query: textResponse, lang: "es", sessionId: localStorage.getItem('sessionId') }),
                    success: function(data) {
                        console.log("RESP CODE: "+data.status.code)
                        if(data.status.code==206 && reloadTry<3){
                            reloadTry++;
    ...
    function insertCarousel(text, time){
        ...
        control = '<li style="width:100%;">' +
                        '<div class="carousel" id="carousel'+idCar+'">' +
                            text +
                        '</div>' +
                    '</li>';
        setTimeout(
