    ...
    var baseUrl = "https://dialogflow.googleapis.com/v2/projects/consultas-sau/agent/sessions/";
    var tokenUrl = "https://v2-eva.herokuapp.com/v2";//"https://chatbotsau-220511.appspot.com/v2";
    var first = false;
    var idCar = 0;
...
$(document).ready(function() {
    ...
    if(!localStorage.getItem('alertEva')){
        $('#modalEvaInfo').modal({show: true});

        //Tiempo, en minutos, para mostrar el modal de inicio
        var seconds = 10;
    ...
    function getToken(rel){
        ...
        if(reloadTry<3){
            ...
            insertChat("bot", "Hubo un problema con la conexión inténtalo de nuevo más tarde");
            insertChat("bot", "Disculpa por las molestias");
        }
    }

    //Llamamos al evento de bienvenida, solo funciona la primera vez que se abre el chatbot
    function callToEvent(event, rel){
        var  reloadTry = rel;
        manageTyping(1);
        ...
        $.ajax({
            type: "POST",
            url: baseUrl + localStorage.getItem('sessionId') +":detectIntent",
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            headers: {
                "Authorization": "Bearer " + localStorage.getItem('accessToken')
            },
            data: JSON.stringify({ queryInput: {event: { languageCode: "es", name: event } } }),
            statusCode: {
                206: function() {
                    if(reloadTry<3){
                        reloadTry++;
                        callToEvent(event, reloadTry);
                    }
                    return;
                },
                401: function() {
            ...
            success: function(data) {
                console.log("EVENT RESP CODE: "+JSON.stringify(data))
    ...
    function send(text, rel) {
        ...
        $.ajax({
            ...
            data: JSON.stringify({ queryInput: {text: { languageCode: "es", text: text } } }),
            statusCode: {
                206: function() {
                    if(reloadTry<3){
                        reloadTry++;
            ...
            success: function(data) {
                console.log("RESP CODE: "+JSON.stringify(data))
                setResponse(JSON.stringify(data, undefined, 2), data);
            },
            error: function() {
                manageTyping(0);
    ...
    function setResponse(val, data) {
        ...
        var response = data.queryResult;

        //if(response.metadata.webhookUsed){
        if(true){

            response.fulfillmentMessages.forEach(function(res) {

                if(res.text && res.platform==null){
                    var value = res.text.text[0].replace(/\n/g, "<br>");
                    insertChat("bot", value, timeDelay);
                    timeDelay+=getDelay();
                }
                /*else if(res.simpleResponses){
                    var value = res.simpleResponses.simpleResponses[0].textToSpeech.replace(/\n/g, "<br>");
                    insertChat("bot", value, timeDelay);
                    timeDelay+=getDelay();
                }*/
                else if(res.carouselSelect){
                    var cards = "";
                    res.carouselSelect.items.forEach(function(e) {
                        var b = "";
                        b += '<a onclick="sendButton(\''+e.info.key+'\')" class="btn btn-bs">'+e.info.key+'</a>';
                        cards += '<div class="card"><img class="card-img-top img-responsive" src="'+e.image.imageUri+'" alt="'+e.image.accessibilityText+'"><div class="card-body"><h4 class=" h5 card-title">'+e.title+'</h4><p class="card-text">'+e.title+'</p><div class="btn-card">'+b+'</div></div><div class="clearfix"></div></div>';
                    });

                    if(res.carouselSelect.items.length==1)
                    ...
                    var button = "";
                    res.suggestions.suggestions.forEach(function(e) {
                        button += '<a style="margin:3px;" class="btn btn-bs" onclick="sendButton(\''+e.title+'\')">'+e.title+'</a>';
                    });
            ...
            /*
            for(i=0; i<response.fulfillmentMessages.length;i++){

                switch (response.fulfillmentMessages[i].type){
                    //TEXT
                    case 0:
